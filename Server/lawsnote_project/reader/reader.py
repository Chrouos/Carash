...
class CollateFunction:
    def __init__(self, config, formatter, mode):
        self.config = config
        self.formatter = formatter
        self.mode = mode

    def __call__(self, data):
        return self.formatter[self.mode].process(data, self.config, self.mode)

def init_formatter(config, task_list, *args, **params):
    for task in task_list:
        formatter[task] = form.init_formatter(config, task, *args, **params)
        collate_fn[task] = CollateFunction(config, formatter, task)
...
def init_predict(config, *args, **params):
    init_formatter(config, ["test"], *args, **params)
    predict_dataset = init_one_dataset(config, "test", *args, **params)
...
def init_valid_dataset(config, *args, **params):
    init_formatter(config, ["valid"], *args, **params)
    test_dataset = init_one_dataset(config, "valid", *args, **params)
