...
def init_all(config, gpu_list, checkpoint, mode, *args, **params):
    ...
    if len(gpu_list) > 0:
        ...
        logger.info("Number of Total Parameters: " + str(pytorch_total_params))
        pytorch_total_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
        logger.info("Number of Trainable Parameters: " + str(pytorch_total_params))
        model = model.cuda()
...
def init_all_test(config, gpu_list, checkpoint, mode, *args, **params):
    result = {}
    ...
    try:
        ...
        # Remove this line if you prefer strict=True
        if "bert.module.LoRa.base_model.model.embeddings.position_ids" in parameters["model"]:
            del parameters["model"]["bert.module.LoRa.base_model.model.embeddings.position_ids"]
        
        model.load_state_dict(parameters["model"], strict=False)  # Set strict=False if you want to ignore unexpected keys
