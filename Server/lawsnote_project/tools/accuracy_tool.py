...
def print_micro_result(mode, res):
    precision = []
    recall = []
    f1 = []
    total = {"TP": 0, "FP": 0, "FN": 0, "TN": 0}
    print(mode + " micro result: ")
    for a in range(0, len(res)):
        total["TP"] += res[a]["TP"]
        total["FP"] += res[a]["FP"]
        total["FN"] += res[a]["FN"]
        ...
        recall.append(r)
        f1.append(f)
        print(a, round(p, 3), round(r, 3), round(f, 3))
...
def log_distance_accuracy_function(outputs, label, config, result=None):
    ...
    result[1] += float(torch.sum(torch.log(torch.abs(outputs - label) + 1)))
...
def multi_label_accuracy_sec(outputs, label, config, result=None):
    if len(label[0]) != len(outputs[0]):
        raise ValueError('Input dimensions of labels and outputs must match.')
    ...
    for i in range(nr_classes):
        outputs1 = (outputs[:, i] >= 0.5).long()
        labels1 = (label[:, i].float() >= 0.5).long()
        total += int((labels1 * outputs1).sum())
        total += int(((1 - labels1) * (1 - outputs1)).sum())
        # print(i)
        # print(outputs[:, i])
