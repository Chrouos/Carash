...
exports.generateQuestionKeyModule = (responseData, questionExplain) => {
    var questionprompt = `你是一位交通諮詢代理人，使用溫柔的口氣表達對當事人發生的事感到惋惜，現在已經結束了諮詢對話，請當事人再次確認表格是否正確，若無要更改部分，請指示他'請點選下一步'。\n` +
    ...
    for (const key in responseData.incidentJson["車禍發生事故"]) {
        if (!responseData.incidentJson["車禍發生事故"][key]) {
            ...
            questionprompt = `作為一名「車禍事故顧問專家」，你的兩項主要任務是：\n1. 根據專家上一個問題與當事人的回答，適當的回應當事人。\n2. 根據以下[下一個問題與問題解釋]中的內容，你必須詢問一個${key}相關的問題。\n\n當當事人回答問題後，首先根據當事人所提供的信息給出具體且個性化的建議。若當事人的回答似乎與問題不相關，以友善且專業的態度指導當事人，但同時避免使用重複的短語，並嘗試提供新的引導或建議。\n\n每次當事人回答完畢後，請根據以下[下一個問題與問題解釋]中提供的內容，提出一個有關${key}的下一個問題。確保你的問題清晰且有針對性，以幫助當事人更好地理解並回答，這時你只需要問一個問題就好，不要有過多的贅字與解釋。\n\n` +
            `- [專家上一個問題]: \"${responseData.ccgCurrentQuestion}\"\n\n` +
            `- [當事人回答]: \"${responseData.userDescription}\"\n\n` +
            `- [下一個問題與問題解釋]: '${key}'的意思是'${questionExplain[key]}'\n\n` +
