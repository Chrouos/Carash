...
def converted_json(preFile = "test_event.json", convertedFile = './converted_test_event.json'):
    with open("./" + preFile, 'r', encoding="utf-8") as jsonFile:
        raw_json = jsonFile.read() # 單行 json
        
        raw_json_str_list = raw_json.strip().split("\n")                            # * 換行符號拆分，list儲存
        raw_json_list = [json.loads(raw_str) for raw_str in raw_json_str_list]      # * 變成 JSON 格式
        json_array_str = json.dumps(raw_json_list, indent=2, ensure_ascii=False)    # * 將Python字典列表轉換回一個單一的JSON陣列字符串
    ...
    with open('./' + fileName, 'r', encoding="utf-8") as jsonFile:
        ...
        for content in jsonContents:
            ...
            collection.add(
                ...
                metadatas=[{"happened": content['happened'], "money": content['money']}]
            )
            
            ids_counts += 1
        

chroma_client = chromadb.HttpClient(host='140.115.54.36', port=8000)
